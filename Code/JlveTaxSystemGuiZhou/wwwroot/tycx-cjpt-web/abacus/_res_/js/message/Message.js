...
(function() {
	...
	var dftCfg = {
		...
		okBtnClickableDelay: 0		// 2017-10-17 add by dfw. 新增按钮可点击延迟。效果：（5）确定
	},curCfg = {};
	
	var hasOkBtnClickableDelay = false;
	var tempCountdown = 0;
	var finishedInit = false;
	var okBtnDelayInterval;
	var delayColor = "#BEBEBE";
	var sourceColor;
	var btnClickEvent;
	...
	(function() {
		...
		var init = function() {
			...
			// 2017-10-17 add by dfw
			if (hasOkBtnClickableDelay) {
				clearInterval(okBtnDelayInterval);
			}
			
			hasOkBtnClickableDelay = true;
			tempCountdown = curCfg.okBtnClickableDelay;
			var btn = document.getElementById(btnCache[btnIndex = 0].id);
			// btnClickEvent = btn.onClick;
			// btn.onClick = null;
			btn.disabled=true;
			sourceColor = btn.style.backgroundColor;
			btn.style.backgroundColor = delayColor;
			// console.log(document.getElementById(btnCache[btnIndex = 0].id).value);
			// console.log(curCfg.okBtnClickableDelay);
			okBtnDelayInterval = setInterval(okBtnDelayFunc, 1000);
		}; 
		var okBtnDelayFunc = function() {
			// 仅在显示后才进行倒数操作。
			if (finishedInit) {
				var btn = document.getElementById(btnCache[btnIndex = 0].id);
				if (tempCountdown > 0) {
					// console.log(tempCountdown--);
					btn.value = " (" + tempCountdown-- + ") 确定 ";
					btn.disabled = true;
				} else {
					clearInterval(okBtnDelayInterval);
					btn.value = " 确定 ";
					btn.style.backgroundColor = sourceColor;
