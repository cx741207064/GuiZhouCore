...
//                a.style.left = e.left + (b ? 0 : c.scroll(1)) + "px",
                ...
                if (boxWidth + leftWidth >= bodyWidth) { 	
                    var leftPX = e.right - boxWidth;	 	//出现了右边溢出，右对齐
                } else {
                    var leftPX = leftWidth; 	//未出现溢出，左对齐
                }
                ...
                c.addClass(d.body, "laydate_body"),
                    ...
                    c.on(h.oclear, "click",
                        function() {
                            ...
                            if(c.options.jpath) {
                            	if(parent.formulaEngine) parent.formulaEngine.apply(c.options.jpath, "");
                            	if(window.setFormulaEngine) setFormulaEngine.apply(c.options.jpath, "");
                            	if(window.viewEngine) viewEngine.tipsForVerify(document.body);
                            	if(window.setViewEngine) setViewEngine.tipsForVerify(document.body);
                            }
                            c.close();
                        }),
