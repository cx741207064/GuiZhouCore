...
function(a, b) {
    ...
    f = {
        ...
        alertEx: function(a, b, d, stack){
        	if (typeof a == "object") {
        		// alertEx({content:'提示信息',stack:'堆栈'});
        		stack = a.stack;
        		a = a.content;
        	} else if(typeof b == "object" && typeof d == "string" ) {
        		// alertEx('提示信息',{icon:5},'堆栈');
        		stack = d;
        		d = null;
    ...
    g.pt.vessel = function(a, b) {
        ...
        return f.zIndex = g,
        b([f.shade ? '<div class="layui-layer-shade" id="layui-layer-shade' + d + '" times="' + d + '" style="' + ("z-index:" + (g - 1) + "; background-color:" + (f.shade[1] || "#000") + "; opacity:" + (f.shade[0] || f.shade) + "; filter:alpha(opacity=" + (100 * f.shade[0] || 100 * f.shade) + ");") + '"></div>': "", '<div class="' + h[0] + " " + (h.anim[f.shift] || "") + (" layui-layer-" + e.type[f.type]) + (0 != f.type && 2 != f.type || f.shade ? "": " layui-layer-border") + " " + (f.skin || "") + '" id="' + h[0] + d + '" type="' + e.type[f.type] + '" times="' + d + '" showtime="' + f.time + '" conType="' + (a ? "object": "string") + '" style="z-index: ' + g + "; width:" + f.area[0] + ";height:" + f.area[1] + (f.fix ? "": ";position:absolute;") + '">' + (a && 2 != f.type ? "": k) + '<div id="' + (f.id || "") +'"'+(f.stack ? ' ondblclick="showStack(this);"' : '') + ' class="layui-layer-content' + (0 == f.type && -1 !== f.icon ? " layui-layer-padding": "") + (3 == f.type ? " layui-layer-loading" + f.icon: "") + '">' + (0 == f.type && -1 !== f.icon ? '<i class="layui-layer-ico layui-layer-ico' + f.icon + '"></i>': "") + (1 == f.type && a ? "": f.content || "") + '</div><span class="layui-layer-setwin">' +
        function() {
        ...
        + (f.stack ? '<div id="lystackid" class="layui-stack" style="display:none;">'
	      + f.stack
	      + '</div>' : '')
        + "</div>"], k),
        c
    ...
    g.pt.callback = function() {
        ...
        function() {
    		...
    		//如果实现了按钮对应的索引事件即（btn+index 如：btn1:function(){}）则执行该事件，舍弃默认事件；如果没有实现则按如下默认规则：第一个按钮为yes事件，最后一个按钮为cancel事件
        	var e=c(this).index();
            var be=g.btn.length;
            if(g["btn"+(e+1)]&&typeof g["btn"+(e+1)]==="function"&&g["btn2noclose"] == 1){
            	g["btn"+(e+1)](b.index,d);
            }else{
            	g["btn" + (e + 1)] && g["btn" + (e + 1)](b.index, d),
            	0===e?g.yes?g.yes(b.index,d):f.close(b.index):1===e?a():g["btn"+(e+1)]||f.close(b.index);
    ...
    f.close = function(a) {
        ...
        if (b[0]) {
            ...
            c("#layui-layer-moves").remove(),
            c("#layui-layer-shade" + a).remove(),
            f.ie6 && e.reselect(),
            e.rescollbar(a),
            c(document).off("keydown", e.enter),
