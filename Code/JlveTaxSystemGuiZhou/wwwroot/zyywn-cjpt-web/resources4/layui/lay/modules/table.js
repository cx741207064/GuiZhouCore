...
layui.define(["laytpl", "laypage", "layer", "form", "util"], function (e) {
	...
	var t = layui.$,
		...
		z = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">', "{{# if(d.data.toolbar){ }}", '<div class="layui-table-tool">', '<div class="layui-table-tool-temp"></div>', '<div class="layui-table-tool-self"></div>', "</div>", "{{# } }}", '<div class="layui-table-box">', "{{# if(d.data.loading){ }}", '<div class="layui-table-init" style="background-color: #f7f7f7;">', '<i class="layui-icon layui-icon layui-icon-gif"></i>', "</div>", "{{# } }}", "{{# var left, right; }}", '<div class="layui-table-header">', _(), "</div>", '<div class="layui-table-body layui-table-main">', E, "</div>", "{{# if(left){ }}", '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', _({
			fixed: !0
	...
	F.prototype.config = {
		...
		}() - 0 - 1;  //e.getScrollWidth(e.layMain[0]) 调整为0 
		...
		r(), r(!0), e.autoColNums = a, e.eachCols(function (i, a) {
			...
			a.colGroup || a.hide || (0 === a.width ? e.getCssRule(t.index + "-" + a.key, function (e) {
				...
				e.style.width = Math.floor(parseFloat(a.width) / 100*o) +'px';//修改ie8百分比问题 o-nj
		...
		var i = this,
			a = i.config;
		a.loading && (e ? (i.layInit && i.layInit.remove(), delete i.layInit, i.layBox.find(g).remove()) : (i.layInit = t(['<div class="layui-table-init">', '<i class="layui-icon layui-icon-gif layui-icon"></i>', "</div>"].join("")), i.layBox.append(i.layInit)))
