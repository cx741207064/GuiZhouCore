...
if (!!window.ActiveXObject || "ActiveXObject" in window){
	 ...
	 setTimeout(selectPH,200)
	

 $("body").on("blur",".layui-form-select .layui-input",function(){
	   var value=$(this).attr("placeholder")
	   var val=$(this).val()
	   if(val==value || val==""){ 
		           
		            if(value !== ""){ $(this).attr("value",value).addClass("input-on")}
 ...
 $("body").on("click",".layui-form-select .layui-input",function(){
	 var value=$(this).attr("placeholder")
	 var val=$(this).val() 
	 if(val==value){
                    $(this).attr("value","").removeClass("input-on")
                }else{
					if(val==""){$(this).removeClass("input-on")}
                   }
...
$(function(){
		...
		var value=$(this).find(".layui-input").attr("placeholder");
		var val=$(this).find(".layui-input").attr("value");
		
		if(val!==""){
	                 $(this).find(".layui-input").attr("value",val)
		            }else{
			              $(this).find(".layui-input").attr("value",value).addClass("input-on")  
		                  }
